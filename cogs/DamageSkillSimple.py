...
from . import custom_embed_skill, tokkoulist, command_log, error_log
...
job1 = [f'{emoji_1}ノービス', f'{emoji_2}ソルジャー', f'{emoji_3}アーチャー', f'\n{emoji_4}マジシャン', f'{emoji_5}ウォーリア',
        f'{emoji_6}ボウマン', f'\n{emoji_7}メイジ']
job2 = [f'{emoji_1}ロウニン', f'{emoji_2}ドラゴンキラー', f'{emoji_3}プリースト', f'\n{emoji_4}スカーミッシャー', f'{emoji_5}ハグレモノ',
        f'{emoji_6}ルーンキャスター', f'\n{emoji_7}スペランカー', f'{emoji_8}アーサー', f'{emoji_9}シーカー']
job3 = [f'{emoji_1}スカルピアサー', f'{emoji_2}バタフライシーカー', f'{emoji_3}アレイスター', f'\n{emoji_4}ダークブラスター 1',
        f'{emoji_5}ダークブラスター 2']


class DamageSkillSimple(commands.Cog):
    def __init__(self, bot: commands.Bot):
        self.bot = bot

    @commands.command(name='sskill1', aliases=['ss1'])
    async def skill_simple_embed_1(self, ctx: commands.Context, raw: float, os: int = 0, *args: str):
        ...
        embeds = custom_embed_skill(ctx=ctx, raw=raw, dmg=dmg, sp_dmg=dmg, os=os, osval=osval, stone_list=list(args),
                                    stone_val=alpha)
    ...
    @commands.command(name='sskill2', aliases=['ss2'])
    async def skill_simple_embed_2(self, ctx: commands.Context, raw: float, os: int = 0, *args: str):
        args = list(set(args) & {'1', '2', '3', '4', '4_5', '4.5', '5', 'leg'})
        if len(args) == 0:
            args = 'なし'
        ...
        embeds = custom_embed_skill(ctx=ctx, raw=raw, dmg=dmg, sp_dmg=dmg, os=os, osval=osval, stone_list=list(args),
                                    stone_val=alpha)
        sent_message = await ctx.send(', '.join(job1), embed=embeds[7])
    ...
    @commands.command(name='sskill3', aliases=['ss3'])
    async def skill_simple_embed_3(self, ctx: commands.Context, raw: float, os: int = 0, *args: str):
        args = list(set(args) & {'1', '2', '3', '4', '4_5', '4.5', '5', 'leg'})
        if len(args) == 0:
            args = 'なし'
    ...
    @skill_simple_embed_1.error
    @skill_simple_embed_2.error
    @skill_simple_embed_3.error
    async def skill_error(self, ctx: commands.Context, error):
        await error_log(bot=self.bot, ctx=ctx, error=error)


def setup(bot: commands.Bot):
    bot.add_cog(DamageSkillSimple(bot=bot))
